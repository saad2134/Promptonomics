...
(function() {
    ...
    // Test 7: Check CO2/H2O calculation specifically
    console.log('Test 7: Checking CO2/H2O calculation...');
    if (typeof PromptOptimizer !== 'undefined') {
        try {
            const optimizer = new PromptOptimizer();
            const testPrompt = "Please can you help me write a very good and nice response to this request that I have for you which is really quite important?";
            const optimized = optimizer.optimizePrompt(testPrompt);
            const savings = optimizer.calculateSavings(testPrompt, optimized);
            
            console.log('📝 Test prompt length:', testPrompt.length, 'characters');
            console.log('✨ Optimized prompt length:', optimized.length, 'characters');
            console.log('💾 Savings calculated:', savings.saved);
            
            if (parseFloat(savings.saved.co2) > 0) {
                console.log('✅ PASS: CO2 savings calculated:', savings.saved.co2, 'g');
            } else {
                console.log('❌ FAIL: CO2 savings is zero or negative');
                hasIssues = true;
            }
            
            if (parseFloat(savings.saved.water) > 0) {
                console.log('✅ PASS: Water savings calculated:', savings.saved.water, 'ml');
            } else {
                console.log('❌ FAIL: Water savings is zero or negative');
                hasIssues = true;
            }
            ...
            if (typeof chrome !== 'undefined' && chrome.storage) {
                chrome.storage.local.get(['totalSavings'], (result) => {
                    console.log('📊 Current stored savings:', result.totalSavings);
                    
                    if (!result.totalSavings) {
                        console.log('ℹ️ INFO: No savings stored yet. Try optimizing a prompt first.');
                    } else if (result.totalSavings.co2 > 0 || result.totalSavings.water > 0) {
                        console.log('✅ PASS: Savings data found in storage');
                    } else {
                        console.log('❌ FAIL: Savings data is zero in storage');
                        hasIssues = true;
                    }
                });
            }
            ...
            console.log('❌ FAIL: Error testing CO2/H2O calculation:', e.message);
            hasIssues = true;
        }
    } else {
        console.log('❌ FAIL: Cannot test CO2/H2O calculation - PromptOptimizer not loaded');
        hasIssues = true;
    }
    console.log('');
